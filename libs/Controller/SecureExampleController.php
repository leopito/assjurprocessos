	...
	public function Login()
	{
		...
		if ($user->Login(RequestUtil::Get('username'), RequestUtil::Get('password')))
		{
			...
			$this->SetCurrentUser($user); 
                        $_SESSION['login'] = RequestUtil::Get('username');
                        $_SESSION['permissao'] = $user->permissao;
                        if($user->permissao == 3){
                            $this->Redirect('User.ListView');
                        }
                        else{
                            $this->Redirect('Processo.ListView');
                        }
		}
		else
		{
			// login failed
			$this->Redirect('SecureExample.LoginForm','Combinação incorreta de usuário e senha');
		}
	}
